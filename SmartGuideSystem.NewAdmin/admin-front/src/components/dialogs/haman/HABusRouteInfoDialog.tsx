...
function HABusRouteInfoDialog({ title, onCancel, busRouteInfo, onOk }: Props) {
  ...
  const formik = useFormik<IHABusInfo>({
    ...
    onSubmit: (values) => {
      ...
      formData.append("dataType", "Route");
      formData.append("busTypeName", values.busTypeName!);
      formData.append("busRouteName", values.busRouteName!);
       ...
       left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] rounded-xl shadow-2xl flex flex-col ">
        ...
        <form
          ...
          <LabelFormik
            ...
            title="버스타입명:"
            className="pt-2"
            errors={formik.errors.busTypeName}
          />
          ...
          <input
            ...
            value={formik.values.busTypeName}
          ...
          <LabelFormik
            ...
            title="버스경로명:"
            className="pt-2"
            errors={formik.errors.busRouteName}
          />

          <input
            name="busRouteName"
            placeholder="버스경로명 입력하세요"
            onChange={formik.handleChange}
            value={formik.values.busRouteName}
            className="px-3 py-2 rounded-md focus:shadow outline-green-500/50 focus:outline"
          />
...
export default function showHABusRouteInfoDialog(
