...
using WeatherAirService.Models;

namespace WeatherAirService.Services
{
    public class WeatherAndAirService
    {
        ...
        public WeatherAndAirService(IConfiguration configuration, ILogger<WeatherAndAirService> logger, IHttpClientFactory httpClientFactory)
        {
            ...
            var key = session["ServiceKey"];
            var nx = int.Parse(session["NX"]);
            var ny = int.Parse(session["NY"]);
            var stationName = session["StationName"];
        ...
        public async Task Request초단기예보()
        {
            ...
            var queryValues = new Dictionary<string, string>()
            {
                ...
                ["numOfRows"] = "100",
                ["dataType"] = "json",
                ["base_date"] = forecastTime.ToString("yyyyMMdd"),
                ["base_time"] = forecastTime.ToString("HH30"),
                ["nx"] = $"{NX}",
                ["ny"] = $"{NY}",
            };
            ...
            if (apiResult != null && apiResult.Response?.Header?.ResultCode == "00")
            {
                ...
                CurrentWeatherAir.pty = pty;
                CurrentWeatherAir.sky = sky;
