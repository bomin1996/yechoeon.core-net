...
import { ISGSCTeamBannerNode } from "../../../../../shares/ISGSCNode";
...
interface Props {
  node: ISGSCTeamBannerNode;
}

export default function TeamBannerNode({ node }: Props) {
  const dialogCtx = useContext(DialogContext);
  if (node.templateName === "banner1") {
    return (
      <div className=" w-full h-full ">
        <ColorInfoButton
          colorStyle={node.color}
          title={node.title}
          ...
          onClick={() => {
            dialogCtx?.pushDialog(
              <TeamModal
                ...
                teamInfo={{
                  name: node.title,
                  officeTel: node.officeTel ?? "",
                  officeFax: node.officeFax ?? "",
  ...
  } else if (node.templateName === "banner2") {
    let cn1 = "bg-red-600";
    let cn2 = "bg-red-400";
