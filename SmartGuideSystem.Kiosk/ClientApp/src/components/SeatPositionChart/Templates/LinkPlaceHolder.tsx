...
import { showSeatPosChartModal } from "src/components/Search";

interface Props {
  className?: string;
  node: ISGSCLinkNode;
  onSelect: (node: ISGSCLinkNode) => void;
}

export default function LinkPlaceHolder({ node, className, onSelect }: Props) {
  ...
  return (
    <div
      className={`w-full h-full flex items-center text-[36px] active:scale-90 text-white font-bold px-6 space-x-4 rounded-full shadow-[0_4px_4px_0px_rgba(0,0,0,0.25)] ${cn1} ${className}`}
      onMouseDown={(ev) => {
        showSeatPosChartModal(dialogCtx!, node.chartId);
      }}
    >
      <svg
        width="49"
